...
{ 
    ...
    public class StringAcceopt
    {
        static void Main()
        {
            Console.WriteLine("Please Enter your FA path:");
            ...
            FA NFA = new FA(NFAS);
            FA DFA = NFAToDFA(NFA);
            bool IsAccepted = IsAcceptedFunc(DFA, input);
            Console.WriteLine(IsAccepted?"Accepted":"Rejected");
        }

        private static bool IsAcceptedFunc(FA DFA, string input)
        {
            int idx = 0;
            string position = DFA.initial_state;
            while(idx<input.Length)
            {
                if(DFA.transitions[position].ContainsKey(input[idx].ToString()))
                {
                    position = DFA.transitions[position][input[idx].ToString()];
                }
                else
                    return false;
                idx++;
            ...
            if(DFA.final_states.Contains(position))
                return true;
            return false;
